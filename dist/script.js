...
function countExp(time) {
  ...
  const resultEN = years ? months ? "".concat(years, " ").concat(years > 1 ? 'years' : 'year', " and ").concat(months - years * 12, " ").concat(months - years * 12 > 1 ? 'months' : 'month') : 'less than 1 month' : "".concat(months, " ").concat(months > 1 ? 'months' : 'month');
  const resultRU = years ? months ? "".concat(years, " ").concat(years > 1 ? years > 4 ? 'лет' : 'года' : 'год', " \u0438 ").concat(months - years * 12, " ").concat(months - years * 12 > 1 ? months - years * 12 > 4 ? 'месяцев' : 'месяца' : 'месяц') : 'меньше 1 месяца' : "".concat(months, " ").concat(months > 1 ? months > 4 ? 'месяцев' : 'месяца' : 'месяц');
...
$(document).ready(function () {
  ...
  if (navigator.userAgent.indexOf(' UBrowser/') >= 0 || navigator.userAgent.indexOf(' UCBrowser/') >= 0) {
    navigator.userAgent.indexOf(' UBrowser/') >= 0 ? console.log('UBrowser: ', navigator.userAgent.indexOf(' UBrowser/') >= 0) : console.log('UCBrowser: ', navigator.userAgent.indexOf(' UCBrowser/') >= 0);
  ...
  $('.portfolio__item').on('click', function (e) {
    if ($(e.target).hasClass('portfolio__item-header') || $(e.target).hasClass('portfolio__item') || $(e.target).hasClass('portfolio__item-name')) {
      $(e.currentTarget).toggleClass('portfolio_open');
    }
  });
  $('#total_work_exp').html(function () {
    const total_dates = $('.experience__dates').map(function (i, el) {
